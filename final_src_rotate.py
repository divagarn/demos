...
class RobotMovement:
    ...
    def rotate_slowly(self, angle):
        """
        Rotate the robot slowly by a given angle using cmd_vel.

        Args:
            angle: The angle to rotate the robot in degrees.
        """
        angular_speed = 0.3  
        angular_tolerance = math.radians(1)  
 
        target_angle = math.radians(angle)

        target_quaternion = quaternion_from_euler(0, 0, target_angle)
        ...
        while abs(current_angle - target_angle) > angular_tolerance:
            ...
            rospy.sleep(0.1) 
            current_angle += angular_speed * 0.1
