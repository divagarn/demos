...
class RobotMovement:
    def __init__(self):
        ...
        rospy.Subscriber("/person_position", String, self.person_lost)
    ...
    def person_detect(self, data_):
        ...
        #global_name = rospy.get_param("/person_follower/tracker","0")
        ...
        if self.depth_ == 100.0 and self.lastDepth!=100.0:
            self.waiting = True
            self.move(self.lastDepth, self.lastangle,True)
          
        self.lastDepth = self.depth_
        self.lastangle = self.angle_
        
    def person_lost(self, data_):
        self.depth_, self.angle_, self.depvel_, self.angvel_ = [float(x) for x in
		                                                data_.data.split(",")]  # This is for the docker run
        if(self.depth_ != 100.0 and self.waiting):
    ...
    def move(self, distance, degree,waitforGoal_ = False):
        """
